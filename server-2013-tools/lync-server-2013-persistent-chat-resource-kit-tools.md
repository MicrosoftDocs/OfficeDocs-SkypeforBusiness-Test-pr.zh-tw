...
ms:mtpsurl: https://technet.microsoft.com/zh-tw/library/JJ945599(v=OCS.15)
ms:contentKeyID: 52056016
ms.date: 09/11/2014
...
The Lync Server 2013 常設聊天室 Resource Kit tools help to make routine tasks easier for IT administrators who deploy and manage Lync Server 2013 常設聊天室伺服器. In addition to installation instructions, this topic describes the purpose of each tool, and examples of its use.

## Installation of the Resource Kit Tools

To install the Lync Server 2013 Resource Kit 工具, download **PersistentChatReskit.msi**. Run **PersistentChatReskit.msi** to do a simple installation. The .msi installs all the tools in the following path: \\**Program Files\\ Microsoft Lync Server 2013\\Persistent Chat Server Resource Kit**. Tools that are self-contained executables are in this folder. Tools that also have files are in their own subfolders.
...
For optimal performance, the Lync Server 2013 Resource Kit 工具 should be installed in the same environment and with the same specifications that are required for Lync Server 2013.

## Resource Kit Tools Overview

Here are the tools that are provided in the Lync Server 2013 常設聊天室 Resource Kit. The following section provides a description of each tool, including requirements and example usage.
...
The AffCheck tool confirms that the 常設聊天室 back-end database user and group affiliation records match that of Active Directory 網域服務.

## Requirements

The tool is installed with the PersistentChatResKit installer on a domain joined machine.

The user account under which the tool is run must have Read access to the 常設聊天室 back-end database and Active Directory Domain Services.
...
The PersistentChatMonitoringSummary tool moves 常設聊天室 monitoring information from the monitoring database into a specified CSV log file.

The CSV file will contain a breakdown of 常設聊天室 sessions by number of total sessions, successful sessions, unexpected failures, expected failures, and a breakdown of the unexpected failures by diagnostic ID, number of failures, and failure description.

## Requirements

Install the 常設聊天室 Resource Kit tools on a domain-joined machine that has access to the Monitoring database.
...
The 常設聊天室 Stress tool provides an easy way to simulate usage of 常設聊天室 to test real-world performance, including varied user models to better fit your expected usage scenarios.

## Requirements

Install the 常設聊天室 Resource Kit tools onto a domain-joined machine that has access to the 常設聊天室 back-end database.
...
Create users and channels for use in the stress run:
  ...
  - Create as many users as your user model calls for, enable them for Lync, and set their 常設聊天室 policy to Enabled.
  ...
  - We recommend creating extra stress rooms. You can create 50,000 rooms with the following Windows PowerShell 命令列介面 command:
    ```C++
...
6.  Change “ConnectionString” to a connection string for your 常設聊天室 back-end database.
...
The ChatUsageReport tool generates an HTML report of 常設聊天室 service usage.

## Requirements

Install the 常設聊天室 Resource Kit tools on a domain-joined machine that has access to the 常設聊天室 back-end database.

The user account under which the tool is run must have Read access to the 常設聊天室 back-end database.

The file, ChatUsageReport.exe.config, must contain a \<connectionStrings\> section defining the connection string to the 常設聊天室 back-end database. The contents of the default config file are included here, for your reference.
...
ScheduleADSyncForPrincipal is a Microsoft SQL Server 2012 script that must be run directly from within SQL Server Management Studio when connected to the 常設聊天室 back-end database. This script enables you to force 常設聊天室 to synchronize its records of a user with those of Active Directory Domain Services, rather than waiting for the scheduled synchronization time.

## Requirements

The user account under which the script is run must have owner access to the 常設聊天室 back-end database.
